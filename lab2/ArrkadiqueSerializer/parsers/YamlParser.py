...
from ArrkadiqueSerializer.parsers import tokens
...
class YamlParser:
    ...
    def _parse_func(self) -> any:
        str = str(self.__from_file[1])
        func_name = str.replace(" ", "").replace("\n", "")
    ...
    def parse(self, source) -> any:
        ...
        self.__from_file = self.__from_file.split("\n")
        print(self.__from_file)
        if self.__from_file[0] == "_func_:":
            print(1234123412351251235)
            self._parse_func()
